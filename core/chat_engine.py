...
class ChatEngine:
    ...
    async def stream_response(self, user_message: str, conversation_context: List[Dict] = None, max_tokens_override: int = None) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            enhanced_conversation_context = self._ensure_conversation_context(conversation_context)
            
            # Step 3: Process with hybrid reasoning
            hybrid_result = await self.hybrid_manager.process_query(user_message, context, enhanced_conversation_context)
            optimized_prompt = hybrid_result.get('optimized_prompt', user_message)
            max_tokens = max_tokens_override or hybrid_result.get('max_tokens', settings.max_tokens)
            
            # Log token override for stories/long content
            if max_tokens_override:
                print(f"📝 Using token override: {max_tokens_override} tokens for enhanced response")
    ...
    def _format_conversation_context_for_prompt(self, conversation_context: List[Dict] = None) -> str:
        ...
        if not conversation_context or len(conversation_context) == 0:
            ...
            if self.current_chat_messages:
                conversation_context = self.current_chat_messages[-6:]  # Last 6 messages
            else:
                return ""
    ...
    def _ensure_conversation_context(self, conversation_context: List[Dict] = None) -> List[Dict]:
