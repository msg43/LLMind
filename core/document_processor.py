...
class DocumentProcessor:
    ...
    async def process_file(self, file: UploadFile) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"✅ Processed {file.filename}: {len(chunks)} chunks, {len(text)} characters")
            return result
            ...
            for i, file in enumerate(files):
                try:
                    ...
                    if extension not in self.supported_extensions:
                        skipped_files += 1
                        print(f"⏭️  Skipping unsupported file: {file.filename} ({extension})")
                        continue
                    ...
                    successful_results.append({
                        ...
                        "chunks": result["chunks"]  # Include chunks for vector store
                    })
                    
                    print(f"✅ Successfully processed: {file.filename} ({result['chunk_count']} chunks)")
