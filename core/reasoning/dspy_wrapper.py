...
if DSPY_AVAILABLE:
        ...
        def __call__(self, prompt: str, **kwargs) -> str:
            ...
            try:
                ...
                if hasattr(self.mlx_manager, "generate_response_sync"):
                    response = self.mlx_manager.generate_response_sync(
                        prompt, max_tokens
                    )
                else:
    ...
    class QueryAnalysis(dspy.Signature):
        ...
        intent = dspy.OutputField(
            desc="Primary intent: factual, conversational, analytical, computational, creative"
        )
        complexity = dspy.OutputField(
            desc="Complexity level: simple, moderate, complex"
        )
        reasoning_type = dspy.OutputField(
            desc="Best reasoning approach: direct, step_by_step, contextual, decomposed"
        )
    ...
    class ResponseOptimization(dspy.Signature):
        ...
        optimized_prompt = dspy.OutputField(
            desc="Optimized prompt for the specific reasoning strategy"
        )
        strategy_params = dspy.OutputField(
            desc="JSON parameters for strategy execution"
        )
    ...
    class DSPyOptimizedReasoner(dspy.Module):
        ...
        def forward(
            self, query: str, context: str = "", reasoning_history: List[Dict] = None
        ):
                ...
                return {
                    "analysis": {
                        ...
                        "confidence": (
                            ...
                            if analysis.confidence.replace(".", "", 1).isdigit()
                            else 0.7
                        ),
                    },
...
class DSPyWrapper:
    ...
    async def optimize_for_examples(self, examples: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            try:
                optimized_reasoner = teleprompter.compile(
                    self.reasoner, trainset=training_examples
                )
                self.reasoner = optimized_reasoner
                self.reasoner.compiled = True

                print(
                    f"✅ DSPy optimization completed with {len(training_examples)} examples"
                )
                return True
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        stats["optimization_status"] = (
            "compiled"
            if hasattr(self.reasoner, "compiled") and self.reasoner.compiled
            else "uncompiled"
        )
        return stats
