...
class HybridStackManager:
    ...
    async def process_query(
        self, query: str, context: str = "", conversation_context: List[Dict] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            conversation_history = self._format_conversation_context(
                conversation_context
            )

            # PERFORMANCE OPTIMIZATION: Check FastPath first for ultra-simple queries
            fastpath_confidence = await self.strategies["fast_path"].should_handle(
                query, context
            )
            ...
            if fastpath_confidence > 0.95:
                print(
                    f"🚀 Ultra-fast bypass: FastPath confidence={fastpath_confidence:.3f}"
                )
            ...
            if self.enable_dspy and self.dspy_wrapper:
                try:
                    dspy_analysis = await self.dspy_wrapper.analyze_query(
                        query, context
                    )
                    if dspy_analysis:
                        analysis.update(dspy_analysis)
                        print(
                            f"🧠 Query analyzed: {analysis.get('intent', 'unknown')} | {analysis.get('complexity', 'unknown')} | {time.time() - start_time:.3f}s"
                        )
                    else:
            ...
            selected_strategy, confidence = await self._select_strategy(
            ...
            if conversation_history:
                print(
                    f"💬 Using conversation context: {len(conversation_history)} characters"
                )
    ...
    async def _select_strategy(
        self, query: str, context: str, analysis: Dict[str, Any]
    ) -> Tuple[BaseReasoningStrategy, float]:
    ...
    async def _update_performance_stats(
        self, strategy_name: str, processing_time: float, success: bool
    ):
        ...
        if recent_times:
            self.stats["average_processing_time"] = sum(recent_times) / len(
                recent_times
            )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dspy_stats": (
                self.dspy_wrapper.get_performance_stats() if self.enable_dspy else {}
            ),
        ...
        try:
            ...
            for example in examples:
                ...
                if query:
                    ...
                    analysis = (
                        await self.dspy_wrapper.analyze_query(query, context)
                        if self.enable_dspy
                        else {}
                    )
                    selected_strategy, confidence = await self._select_strategy(
            ...
            print(
                f"📊 Current accuracy: {accuracy:.2%} ({matches}/{len(analysis_results)})"
            )
            ...
            if self.enable_dspy and len(examples) >= 5:
                dspy_examples = [
                    {"query": ex["query"], "context": ex.get("context", "")}
                    for ex in examples
                ]
                optimization_success = await self.dspy_wrapper.optimize_for_examples(
                    dspy_examples
                )
    ...
    async def update_configuration(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if (
                "current_stack" in config
                and config["current_stack"] in self.hybrid_stacks
            ):
                self.current_stack = config["current_stack"]

            # Update strategy configurations
            if "strategy_configs" in config:
                for strategy_name, strategy_config in config[
                    "strategy_configs"
                ].items():
                    if strategy_name in self.strategies:
                        await self.strategies[strategy_name].update_config(
                            strategy_config
                        )
