...
class ReActStrategy(BaseReasoningStrategy):
    ...
    async def should_handle(
        ...
        indicator_score = sum(
            1 for indicator in self.react_indicators if indicator in query_lower
        ) / len(self.react_indicators)
    ...
    async def process_query(
        ...
        result["optimized_prompt"] = self._create_react_prompt(
            query, context, reasoning_plan
        )
    ...
    async def _create_reasoning_plan(
    ...
    def _create_react_prompt(
        self, query: str, context: str, reasoning_plan: List[Dict[str, str]]
    ) -> str:
        ...
        if context:
            prompt_parts.extend(["Context information:", context, ""])

        prompt_parts.extend(["Reasoning approach:", "Think through this step-by-step:"])

        for i, step in enumerate(reasoning_plan, 1):
            prompt_parts.append(f"{i}. {step['action']} - {step['focus']}")

        prompt_parts.extend(["", "Provide your reasoning and conclusion:"])
