...
async def lifespan(app: FastAPI):
    ...
    async def initialize_components():
        ...
        try:
            ...
            logger.info(
                f"🎤 Voice features: {'enabled' if settings.enable_voice else 'disabled'}"
            )
            logger.info(
                f"🧠 Hybrid reasoning: {'enabled' if settings.enable_dspy else 'disabled'}"
            )
    ...
    try:
        while True:
            ...
            if message_data["type"] == "chat":
                ...
                if (
                    not hasattr(chat_engine, "hybrid_manager")
                    or not chat_engine.hybrid_manager
                ):
                ...
                try:
                    ...
                    if metadata.get("isStoryRequest", False):
                        ...
                        logger.info(
                            f"📝 Story request detected - using {max_tokens_override} tokens"
                        )
                    ...
                    await websocket.send_text(json.dumps({"type": "chat_end"}))
                except Exception as e:
                ...
                try:
                    ...
                    await websocket.send_text(
                        json.dumps({"type": "voice_transcription", "text": text})
                    )
                except Exception as e:
                ...
                try:
                    ...
                    await websocket.send_text(
                        json.dumps({"type": "tts_audio", "audio": audio_data})
                    )
                except Exception as e:
                ...
                logger.info("📡 WebSocket test message handled")

            else:
                # Handle unknown message types
                logger.warning(
                    f"⚠️ Unknown WebSocket message type: {message_data.get('type', 'undefined')}"
                )
                await websocket.send_text(
                    json.dumps(
                        {
...
async def get_chat_history():
    ...
    try:
        ...
        return {"status": "success", "history": history, "stats": stats}
    except Exception as e:
...
async def get_chat_list():
    ...
    try:
        ...
        return {"status": "success", "chats": chats, "total_count": len(chats)}
    except Exception as e:
...
async def export_chat_markdown(chat_id: str):
    ...
    try:
        ...
        chat_data = await chat_engine.load_chat(chat_id)
        safe_title = "".join(
            c for c in chat_data["title"] if c.isalnum() or c in (" ", "-", "_")
        ).rstrip()
...
async def start_new_chat(chat_data: Dict[str, Any] = None):
    ...
    try:
        ...
        return {"status": "success", "chat_id": chat_id, "message": "New chat started"}
    except Exception as e:
...
async def get_reasoning_stacks():
    ...
    try:
        ...
        return {"status": "success", "stacks": stacks, "current_stack": current}
    except Exception as e:
...
async def optimize_reasoning(optimization_data: Dict[str, Any]):
    ...
    try:
        ...
        if success:
            return {
                "status": "success",
                "message": "Reasoning system optimized successfully",
            }
        else:
            return {
                "status": "warning",
                "message": "Optimization completed with warnings",
            }
    except Exception as e:
...
async def create_custom_reasoning_stack(stack_data: Dict[str, Any]):
    ...
    try:
        ...
        if success:
            return {
                "status": "success",
                "message": f"Custom stack '{name}' created successfully",
            }
        else:
...
async def get_libraries():
    """Get list of all document libraries"""
    try:
        libraries = await vector_store.list_libraries()
...
async def create_library(name: str = Form(...), description: str = Form("")):
    ...
    try:
        ...
        # Clean the name (no special characters)
        clean_name = "".join(
        ...
        result = await vector_store.create_library(clean_name, description.strip())
...
async def delete_library(library_name: str):
    ...
    try:
        result = await vector_store.delete_library(library_name)
...
@app.post("/api/libraries/{library_name}/activate")
async def set_active_library(library_name: str):
    """Set the active library for searches"""
    try:
        result = await vector_store.set_active_library(library_name)
        ...
        logger.error(f"Error setting active library: {e}")
        raise HTTPException(status_code=500, detail=str(e))


# === Document Management ===
@app.post("/api/documents/upload")
async def upload_document(file: UploadFile = File(...), library: str = Form("default")):
    """Upload and process a document to specified library"""
    try:
        result = await document_processor.process_file(file)

        # Add to vector store in specified library
        await vector_store.add_documents(
            [{"text": result["text"], "metadata": result["metadata"]}], library
        )

        return {"status": "success", "library": library, **result}
    except Exception as e:
...
async def remove_monitored_folder(folder_data: Dict[str, str]):
    ...
    try:
        ...
        if result["status"] == "success":
            ...
            logger.warning(
                f"⚠️  Failed to remove monitored folder: {result['message']}"
            )
...
async def get_models():
    ...
    try:
        ...
        return {"status": "success", "models": models, "current_model": current}
    except Exception as e:
...
async def test_huggingface_token(token_data: Dict[str, str]):
    ...
    try:
        ...
        return {"status": "error", "message": f"Error testing token: {str(e)}"}
...
async def update_settings(settings_data: Dict[str, Any]):
    ...
    try:
        ...
        if "top_k_results" in settings_data:
            settings.top_k_results = max(
                1, min(20, int(settings_data["top_k_results"]))
            )
...
async def run_profiling_analysis(
    ...
    try:
        ...
        hardware_profile = (
            "m2_max" if memory_gb >= 64 else "m2" if memory_gb >= 32 else "m1"
        )
        recommendations = optimizer.get_model_recommendations(
            hardware_profile, memory_gb
        )
        ...
        for model in models:
            try:
                ...
                if results:
                    ...
                    avg_memory = sum(r.memory_usage["used_gb"] for r in results) / len(
                        results
                    )
