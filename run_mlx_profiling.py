...
class MLXPerformanceSuite:
    ...
    async def run_comprehensive_analysis(
        self, model_name: str = None, quick_mode: bool = False
    ):
        ...
        if not quick_mode:
            ...
            memory_results = self.profiler.memory_stress_test(
                test_scenarios["simple"], max_iterations=5
            )
        ...
        recommendations = self.optimizer.get_model_recommendations(
    ...
    def _print_analysis_summary(self, analysis: dict):
        ...
        print(
            f"Average Performance: {summary.get('avg_tokens_per_second', 0):.1f} tokens/sec"
        )
        ...
        if bottlenecks:
            ...
            for bottleneck in bottlenecks:
                ...
                print(
                    f"   {severity_emoji} {bottleneck['type']}: {bottleneck['value']}"
                )
...
async def main():
    ...
    if args.optimization_only:
        ...
        results = await suite.run_comprehensive_analysis(
            model_name=args.model, quick_mode=args.quick
        )
