...
def mock_mlx_manager():
    ...
    manager.get_available_models = AsyncMock(
        return_value=[
            {"name": "test-model", "status": "available", "size": "1GB", "type": "test"}
        ]
    )
    ...
    manager.get_memory_usage = Mock(
        return_value={"used_gb": 1.5, "total_gb": 16.0, "percentage": 9.4}
    )
    manager.get_model_info = Mock(
        return_value={"name": "test-model", "status": "loaded"}
    )
...
def mock_vector_store():
    ...
    store.get_size = AsyncMock(
        return_value={"total_documents": 5, "total_vectors": 50, "dimension": 384}
    )
...
def mock_document_processor():
    ...
    processor.get_document_list = AsyncMock(
        return_value=[
            {
                "filename": "test.txt",
                "file_hash": "test_hash",
...
def mock_audio_manager():
    ...
    manager.get_audio_settings = Mock(
        return_value={
