...
class TestModelEndpoints:
    ...
    def test_get_models(self, mock_manager, client):
        ...
        mock_manager.get_available_models = AsyncMock(
            return_value=[{"name": "test-model", "status": "available", "size": "1GB"}]
        )
    ...
    def test_download_model(self, mock_manager, client):
        ...
        response = client.post(
            "/api/models/download", data={"model_name": "test-model"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "success"
...
class TestDocumentEndpoints:
    ...
    def test_upload_documents(self, mock_vector_store, mock_doc_processor, client):
        ...
        mock_doc_processor.process_file = AsyncMock(
            return_value={
                "filename": "test.txt",
                "chunk_count": 1,
                "text_length": 100,
    ...
    def test_get_documents(self, mock_vector_store, mock_doc_processor, client):
        ...
        mock_doc_processor.get_document_list = AsyncMock(
            return_value=[{"filename": "test.txt", "chunk_count": 1}]
        )
        mock_vector_store.get_document_list = AsyncMock(
            return_value=[{"filename": "test.txt", "chunk_count": 1}]
        )
    ...
    def test_delete_document(self, mock_vector_store, mock_doc_processor, client):
        ...
        mock_doc_processor.get_document_by_hash = AsyncMock(
            return_value={"filename": "test.txt"}
        )
...
class TestChatEndpoints:
    ...
    def test_chat_endpoint(self, mock_chat_engine, client):
        ...
        mock_chat_engine.generate_response = AsyncMock(
            return_value={
...
class TestAudioEndpoints:
    ...
    def test_get_voices(self, mock_audio_manager, client):
        ...
        mock_audio_manager.get_voices = AsyncMock(
            return_value=[{"name": "Samantha", "language": "en_US"}]
        )
...
class TestPerformanceEndpoints:
    ...
    def test_performance_metrics(
        self, mock_audio, mock_doc, mock_vector, mock_chat, mock_mlx, client
    ):
        ...
        mock_vector.get_size = AsyncMock(
            return_value={"total_documents": 5, "total_vectors": 50}
        )
...
class TestWebSocketConnection:
    ...
    def test_websocket_chat(self, mock_chat_engine, client):
        ...
        mock_chat_engine.stream_response = AsyncMock(
            return_value=AsyncIterator(["Hello", " there", "!"])
        )

        with client.websocket_connect("/ws") as websocket:
            # Send chat message
            websocket.send_text(
                json.dumps({"type": "chat", "message": "Hello", "context": []})
            )
    ...
    def test_websocket_voice(self, mock_audio_manager, client):
        ...
        with client.websocket_connect("/ws") as websocket:
            # Send voice message
            websocket.send_text(
                json.dumps({"type": "voice", "audio": "base64_audio_data"})
            )
...
class TestErrorHandling:
    ...
    def test_model_error_handling(self, mock_manager, client):
        ...
        response = client.post(
            "/api/models/download", data={"model_name": "invalid-model"}
        )
